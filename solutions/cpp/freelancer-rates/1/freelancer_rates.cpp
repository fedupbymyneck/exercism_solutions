...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double before= 22*daily_rate(hourly_rate);
    return std::ceil(apply_discount(before, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    double price_day= apply_discount(hourly_rate, discount)*8;
